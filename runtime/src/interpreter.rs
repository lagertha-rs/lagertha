...
impl Interpreter {
    ...
    fn interpret_instruction(
        ...
        match instruction {
            Instruction::Aaload => {
                let index = vm.get_stack(&thread_id)?.pop_int_val()?;
                let array_addr = vm.get_stack(&thread_id)?.pop_obj_val()?;
            ...
            Instruction::Aastore => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                let index = vm.get_stack(&thread_id)?.pop_int_val()? as usize;
                let array_addr = vm.get_stack(&thread_id)?.pop_obj_val()?;
            ...
            Instruction::Checkcast(_idx) => {
                //TODO: stub
                let object_ref = vm.get_stack(&thread_id)?.pop_operand()?;
                vm.get_stack(&thread_id)?.push_operand(object_ref)?;
            }
            ...
            Instruction::Anewarray(idx) => {
                let size = vm.get_stack(&thread_id)?.pop_int_val()?;
                if size < 0 {
            ...
            Instruction::ArrayLength => {
                let array_ref = vm.get_stack(&thread_id)?.pop_obj_val()?;
            ...
            Instruction::Astore0 => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                vm.get_stack(&thread_id)?.set_local(0, value)?;
            }
            Instruction::Astore1 => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                vm.get_stack(&thread_id)?.set_local(1, value)?;
            }
            Instruction::Astore2 => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                vm.get_stack(&thread_id)?.set_local(2, value)?;
            }
            Instruction::Astore3 => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                vm.get_stack(&thread_id)?.set_local(3, value)?;
            }
            Instruction::Astore(pos) => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
                vm.get_stack(&thread_id)?.set_local(pos as usize, value)?;
            }
            ...
            Instruction::Castore => {
                let value = vm.get_stack(&thread_id)?.pop_int_val()?;
                let index = vm.get_stack(&thread_id)?.pop_int_val()?;
                let array_ref = vm.get_stack(&thread_id)?.pop_obj_val()?;
                vm.heap
            ...
            Instruction::DupX1 => {
                let value1 = vm.get_stack(&thread_id)?.pop_operand()?;
                let value2 = vm.get_stack(&thread_id)?.pop_operand()?;
                vm.get_stack(&thread_id)?.push_operand(value1)?;
                vm.get_stack(&thread_id)?.push_operand(value2)?;
                vm.get_stack(&thread_id)?.push_operand(value1)?;
            }
            Instruction::Fcmpl => {
                let v2 = vm.get_stack(&thread_id)?.pop_float_val()?;
                let v1 = vm.get_stack(&thread_id)?.pop_float_val()?;
            ...
            Instruction::Fconst0 => {
                vm.get_stack(&thread_id)?.push_operand(Value::Float(0.0))?;
            }
            Instruction::Fconst1 => {
                vm.get_stack(&thread_id)?.push_operand(Value::Float(1.0))?;
            }
            Instruction::Fload0 => {
                let value = *vm.get_stack(&thread_id)?.get_local_float(0)?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
            }
            Instruction::Fload1 => {
                let value = *vm.get_stack(&thread_id)?.get_local_float(1)?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
            }
            Instruction::Fload2 => {
                let value = *vm.get_stack(&thread_id)?.get_local_float(2)?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
            }
            Instruction::Fload3 => {
                let value = *vm.get_stack(&thread_id)?.get_local_float(3)?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
            }
            Instruction::Fload(n) => {
                let value = *vm.get_stack(&thread_id)?.get_local_float(n)?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
            }
            Instruction::Fstore(n) => {
                let value = vm.get_stack(&thread_id)?.pop_float()?;
                vm.get_stack(&thread_id)?.set_local(n as usize, value)?;
            }
            Instruction::Fstore0 => {
                let value = vm.get_stack(&thread_id)?.pop_float()?;
                vm.get_stack(&thread_id)?.set_local(0, value)?;
            ...
            Instruction::Goto(offset) => {
                ...
                *vm.get_stack(&thread_id)?.pc_mut()? = new_pc;
            }
            Instruction::Iadd => {
                let value2 = vm.get_stack(&thread_id)?.pop_int_val()?;
                let value1 = vm.get_stack(&thread_id)?.pop_int_val()?;
            ...
            Instruction::Lcmp => {
                let v2 = vm.get_stack(&thread_id)?.pop_long_val()?;
                let v1 = vm.get_stack(&thread_id)?.pop_long_val()?;
            ...
            Instruction::Ifnull(offset) => {
                let pc = vm.get_stack(&thread_id)?.pc()?;
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref_val()?;
            ...
            Instruction::IfIcmplt(offset) => {
                let pc = vm.get_stack(&thread_id)?.pc()?;
                let v2 = vm.get_stack(&thread_id)?.pop_int_val()?;
                let v1 = vm.get_stack(&thread_id)?.pop_int_val()?;
            ...
            Instruction::IfAcmpEq(offset) => {
                let pc = vm.get_stack(&thread_id)?.pc()?;
                let v2 = vm.get_stack(&thread_id)?.pop_nullable_ref_val()?;
                let v1 = vm.get_stack(&thread_id)?.pop_nullable_ref_val()?;
                let new_pc = if v1 == v2 {
                    Self::branch16(pc, offset)
                } else {
                    pc + instruction.byte_size() as usize
            ...
            Instruction::Ifnonnull(offset) => {
                let pc = vm.get_stack(&thread_id)?.pc()?;
                let obj = vm.get_stack(&thread_id)?.pop_nullable_ref_val()?;
            ...
            Instruction::IfNe(offset) => {
                let pc = vm.get_stack(&thread_id)?.pc()?;
                let i = vm.get_stack(&thread_id)?.pop_int_val()?;
                let new_pc = if i != 0 {
                    Self::branch16(pc, offset)
                } else {
                    pc + instruction.byte_size() as usize
            ...
            Instruction::Instanceof(idx) => {
                ...
                let obj_ref = vm.get_stack(&thread_id)?.pop_nullable_ref_val()?;
            ...
            Instruction::L2i => {
                let v = vm.get_stack(&thread_id)?.pop_long_val()?;
                vm.get_stack(&thread_id)?
            ...
            Instruction::D2l => {
                let v = vm.get_stack(&thread_id)?.pop_double_val()?;
                vm.get_stack(&thread_id)?
                    .push_operand(Value::Long(v as i64))?;
            }
            Instruction::F2i => {
                let v = vm.get_stack(&thread_id)?.pop_float_val()?;
                vm.get_stack(&thread_id)?
                    .push_operand(Value::Integer(v as i32))?;
            }
            Instruction::F2d => {
            ...
            Instruction::Ineg => {
                let v = vm.get_stack(&thread_id)?.pop_int_val()?;
                vm.get_stack(&thread_id)?.push_operand(Value::Integer(-v))?;
            }
            Instruction::I2s => {
                let v = vm.get_stack(&thread_id)?.pop_int_val()?;
                vm.get_stack(&thread_id)?
            ...
            Instruction::Istore0 => {
                let value = vm.get_stack(&thread_id)?.pop_int()?;
                vm.get_stack(&thread_id)?.set_local(0, value)?;
            }
            Instruction::Istore1 => {
                let value = vm.get_stack(&thread_id)?.pop_int()?;
            ...
            Instruction::Istore(idx) => {
                let value = vm.get_stack(&thread_id)?.pop_int()?;
                vm.get_stack(&thread_id)?.set_local(idx as usize, value)?;
            }
            Instruction::Isub => {
                let v2 = vm.get_stack(&thread_id)?.pop_int_val()?;
                let v1 = vm.get_stack(&thread_id)?.pop_int_val()?;
            ...
            Instruction::Iinc(index, const_val) => {
                let value = vm.get_stack(&thread_id)?.get_local_int_val(index)?;
                vm.get_stack(&thread_id)?
            ...
            Instruction::Ldc(idx) | Instruction::LdcW(idx) | Instruction::Ldc2W(idx) => {
                ...
                vm.get_stack(&thread_id)?.push_operand(ldc_operand)?;
            }
            Instruction::New(idx) => {
                let cur_frame_method_id = vm.get_stack(&thread_id)?.cur_frame()?.method_id();
            ...
            Instruction::Ireturn => {
                let ret_value = vm.get_stack(&thread_id)?.pop_int()?;
                vm.get_stack(&thread_id)?.pop_frame()?;
                vm.get_stack(&thread_id)?.push_operand(ret_value)?;
                return Ok(ControlFlow::Break(()));
            }
            Instruction::Areturn => {
                let value = vm.get_stack(&thread_id)?.pop_nullable_ref()?;
            ...
            Instruction::Lreturn => {
                let value = vm.get_stack(&thread_id)?.pop_long()?;
                vm.get_stack(&thread_id)?.pop_frame()?;
                vm.get_stack(&thread_id)?.push_operand(value)?;
                return Ok(ControlFlow::Break(()));
            }
    ...
    fn run_method(
        ...
        if method.is_native() {
            ...
            if let Some(ret) = native(vm, thread_id, args.as_slice())? {
                vm.get_stack(&thread_id)?.push_operand(ret)?;
            }
        } else {
